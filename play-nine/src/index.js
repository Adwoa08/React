...
const Button = (props) => {
    ...
    switch(props.answerIsCorrect){
        ...
        break;
        default:
            button = <button className="btn btn-default" disabled={props.selectedNumbers.length === 0} onClick={() => props.checkAnswer()} >=</button>
        break;
    }
    return (
    ...
    return (
        <div className="col-sm-5">
            {props.selectedNumbers.map((number, i) => {
                return <span key={i} onClick={()=>props.unSelectNumber(number)}>{number}</span>
            })}
        </div>
    );
}
        ...
        <div className="card text-center">
            <div className="span-con">
                {Numbers.list.map((number, i) => 
                    <span key={i} className={isNumberSelected(number)} 
...
class Game extends React.Component {
    ...
    checkAnswer = () => {
        // check to make sure the sum of numbers seleted equals no of stars
        this.setState(prevState => ({
